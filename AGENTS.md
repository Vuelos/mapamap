...
| Place a warp                 | Load an Entities-tab cell (or build one in the factory) into a slot, then LMB paint. The New Warp template places exactly ONE self-destined warp (no return pair). The factory's **NEW MAP** row mints a fresh map on the first free edge and points the draft at it; while the form or Details is open, the destination map + warp # cell are previewed live (pulsing outline + label chip) |
